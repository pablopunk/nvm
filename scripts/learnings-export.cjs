...
function summaryMarkdown(state, storePaths) {
  ...
  const recentTraces = [...traces].sort((left, right) => (right.updatedAt || 0) - (left.updatedAt || 0)).slice(0, 10)
  const lines = [
    '# Learnings Export',
    '',
  ...
  else active.forEach((learning) => lines.push(`- [${learning.kind}] ${learning.summary}`))
  lines.push('', '## Recent Traces', '')
  if (!recentTraces.length) lines.push('_None_')
  else recentTraces.forEach((trace) => lines.push(`- ${trace.title || trace.query || trace.chatId}: ${trace.toolCalls?.length || 0} tools, ${trace.extensionEvents?.length || 0} extension events`))
  lines.push('')
  return `${lines.join('\n')}\n`
}
...
async function main() {
  ...
  const exportDir = path.join(process.cwd(), '.tmp', 'learnings-export', timestampLabel())
...
function parseLearningsForExport(text) {
  ...
  for (const block of blocks) {
    ...
    const kind = heading.startsWith('workflow') ? 'workflow' : heading.startsWith('preference') ? 'preference' : 'environment'
    let current = null
    for (const rawLine of lines) {
      ...
      else if (line.startsWith('Confidence: ')) current.confidence = line.slice('Confidence: '.length).trim()
