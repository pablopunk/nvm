...
const runtimeDirs = [path.join(root, 'dist', 'main'), path.join(root, 'dist', 'preload')]
...
function packageName(specifier) {
  if (!specifier || specifier.startsWith('.') || specifier.startsWith('/') || specifier.startsWith('node:') || specifier.startsWith('file:')) return null
...
function findRuntimeFiles(dir) {
  if (!fs.existsSync(dir)) fail(`missing ${path.relative(root, dir)}; run the build first`)
...
function forbiddenStringReferences(source) {
  ...
  for (const packagePrefix of forbiddenPackages) {
    const pattern = new RegExp(`["']${escapeRegExp(packagePrefix)}(?:/[A-Za-z0-9._/-]*)?["']`, 'g')
...
for (const dir of runtimeDirs) {
  for (const filePath of findRuntimeFiles(dir)) {
    ...
    for (const specifier of importSpecifiers(source)) {
      ...
      if (!pkg) continue
      if (!externalImports.has(specifier)) externalImports.set(specifier, new Set())
      externalImports.get(specifier).add(relativePath)
      const forbidden = forbiddenPackages.find((packagePrefix) => matchesPackage(specifier, packagePrefix))
      if (forbidden) violations.push({ filePath: relativePath, specifier, forbidden })
    }
    for (const forbidden of forbiddenStringReferences(source)) {
      violations.push({ filePath: relativePath, specifier: forbidden, forbidden })
    }
  }
}

for (const violation of violations) {
  fail(`${violation.filePath} imports ${violation.specifier}; ${violation.forbidden} must stay out of packaged main/preload runtime`)
}

if (process.exitCode) process.exit()

const unexpected = Array.from(externalImports.keys())
  .filter((specifier) => !allowedPackages.some((allowed) => matchesPackage(specifier, allowed)))
  .sort()

if (unexpected.length) {
  console.warn('Packaged runtime import check warning: unclassified external runtime imports:')
