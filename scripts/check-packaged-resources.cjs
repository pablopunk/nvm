...
if (!fs.existsSync(path.join(process.cwd(), 'node_modules', 'typescript', 'lib', 'lib.es2022.full.d.ts'))) {
  fail('missing node_modules/typescript/lib/lib.es2022.full.d.ts; run mise exec pnpm -- pnpm install')
}
...
if (!/from:\s*node_modules\/typescript\/lib[\s\S]*to:\s*node_modules\/typescript\/lib[\s\S]*filter:[\s\S]*['"]?lib\*\.d\.ts['"]?/m.test(config)) {
