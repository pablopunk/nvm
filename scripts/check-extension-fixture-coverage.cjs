...
const API_DTS = path.join(ROOT, 'src', 'resources', 'nevermind-extension-api.d.ts');
...
function extractUIMethods(dtsSource) {
  const sf = ts.createSourceFile('api.d.ts', dtsSource, ts.ScriptTarget.Latest, true);
...
function extractNamespaceMembers(decl, namespaces) {
  ...
  const typeLiteral = ts.isTypeAliasDeclaration(decl)
    ? decl.type
    : decl;
...
function collectFixtureCalls(fixtureSource) {
  const sf = ts.createSourceFile('fixture.ts', fixtureSource, ts.ScriptTarget.Latest, true);
  const calls = new Set();
  ...
  function visit(node) {
    ...
    if (ts.isCallExpression(node) && ts.isPropertyAccessExpression(node.expression)) {
      ...
      if (ts.isPropertyAccessExpression(nsAccess) &&
...
function verifyDocList() {
  ...
  if (!section) {
    fail(`Could not find "## Current Fixture" section in ${path.relative(ROOT, DOC_FILE)}`);
    return;
  }
  const bullets = section[1].match(/`ctx\.(ui\.\w+|input\.\w+)`/g) || [];
  const docMethods = new Set(bullets.map((b) => b.replace(/`/g, '').replace('ctx.', '')));
