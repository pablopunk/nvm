...
if (/const\s+INTERNAL_EXTENSIONS\s*=/.test(source)) {
  fail(
    'internal extensions must be registered from lazy factories, not eager instances',
  );
}
...
if (!factories.includes('createAiBuilderExtension'))
  fail('AI Builder factory is not registered');
if (factories.includes('createAiBuilderExtension()'))
  fail('AI Builder must not be constructed in the factory list');

if (
  !/const\s+AI_BUILDER_EXTENSION_ID\s*=\s*['"]nevermind\.ai-builder['"]/.test(
    source,
  )
) {
...
if (
  !/REQUIRED_INTERNAL_EXTENSIONS[\s\S]*AI_BUILDER_EXTENSION_ID/.test(source)
) {
  fail('AI Builder is not required by internal extension assertions');
}

if (
  !/REQUIRED_INTERNAL_COMMANDS[\s\S]*AI_BUILDER_EXTENSION_ID[\s\S]*commandId:\s*['"]ai-chats['"]/.test(
    source,
  )
) {
