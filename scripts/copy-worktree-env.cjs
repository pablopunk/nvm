...
if (samePath(currentRoot, sourceRoot)) fail('Current worktree is already the canonical checkout.')
if (!fs.existsSync(sourceRoot)) fail(`Source checkout does not exist: ${sourceRoot}`)
...
function canonicalCheckout(current) {
  const blocks = git(['worktree', 'list', '--porcelain']).trim().split(/\n\n+/).filter(Boolean)
  ...
  const main = worktrees.find((entry) => entry.branch === 'refs/heads/main' && !samePath(entry.path, current))
  if (main) return main.path

  const other = worktrees.find((entry) => !samePath(entry.path, current))
  if (other) return other.path

  fail('Could not find another checkout. Pass --source /path/to/canonical/checkout.')
}
  ...
  function walk(absDir, relDir) {
    for (const dirent of fs.readdirSync(absDir, { withFileTypes: true })) {
      ...
      if (dirent.isDirectory()) {
        if (['.git', 'node_modules', 'dist', 'release'].includes(dirent.name)) continue
        walk(abs, rel)
        continue
      }
