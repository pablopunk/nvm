...
* **CI-only runtime verification.** Local linters and formatter checks or fixes are allowed. Do not run tests, typechecks, builds, palette debugging, app launches, or other commands that may open or surface the app on the user's computer. Leave runtime verification to CI and report it as intentionally skipped unless the user explicitly asks for local verification.
