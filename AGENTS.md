...
* Treat the palette input and result rows as the canonical UI for short input and choices; avoid stacked form controls. Use an editor for long-form content and native pickers for OS resources. See `docs/palette-first-extension-ui.md`.
...
* Keep `AGENTS.md` concise. Move detailed guides to `docs/`. Document intention, NEVER implementation.
* Before debugging recurring architecture issues, search `docs/solutions/` for prior root causes and verification notes.
* Treat `src/app/resources/nevermind-extension-api.d.ts` as the self-documenting source of truth for the app's extension platform: runtime contract, validation types, author guidance, and AI builder context. Update its types/TSDoc with every API change and do not recreate API reference markdown.
* For selectable or multi-item extension and fixture UI, follow the list-first rule in `docs/list-first-extension-ui.md`; only use its documented exceptions.
* Every extension API endpoint that renders UI must have a dev-only fixture under `src/app/fixtures/`; use `.agents/skills/extensions-ui-test/SKILL.md` for dogfooding and see `docs/extension-api-ui-fixtures.md`.
* Use CSS variables from `src/app/palette/styles.css` (e.g., `--accent-*`, `--radius-*`). Avoid hardcoded colors or radii.

## Verification

* **CI-only runtime verification.** Local linters and formatter checks or fixes are allowed. Do not run tests, typechecks, builds, app launches, or other commands that may open or surface the app on the user's computer. Leave runtime verification to CI and report it as intentionally skipped unless the user explicitly asks for local verification.
...
* **Match checks to scope.** Backend-only changes run `mise exec -- pnpm -C src/backend check` and `mise exec -- pnpm -C src/backend test`. Desktop/shared changes run the relevant root checks, including `mise exec -- pnpm typecheck` and `mise exec -- pnpm test`; use `mise exec -- pnpm verify` for cross-cutting or release-sensitive changes when practical. For applicable palette or interaction changes, also verify navigation, action panels, shortcuts, and dismissal behavior.
