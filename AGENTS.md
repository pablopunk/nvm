...
* **CI-only verification.** Do not run tests, typechecks, linters, builds, palette debugging, app launches, or other commands that may open or surface the app on the user's computer. Leave verification to CI and report local verification as intentionally skipped; only run non-invasive inspection commands unless the user explicitly asks for local verification.
