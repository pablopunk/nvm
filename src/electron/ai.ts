...
function isAssistantErrorEndEvent(
  event: AgentSessionEvent,
): event is AssistantMessageEndEvent {
...
function createNevermindAi(options: NevermindAiOptions) {
  ...
  async function getSession(chatId = 'default') {
    ...
    entry.promise.catch(() => {
      if (sessions.get(chatId) === entry) sessions.delete(chatId);
    });
    ...
    try {
      ...
      unsubscribe = session.subscribe((event) => {
        ...
        if (isToolExecutionStartEvent(event))
          askOptions.onEvent?.({ type: 'tool_start', name: event.toolName });
        ...
        if (isAssistantErrorEndEvent(event))
          askOptions.onEvent?.({
            type: 'error',
            message: event.message.errorMessage || 'AI request failed',
            data: aiLimitNoticeFromError(event.message.errorMessage),
          });
      });
      removeAbortListener = bindAbortSignal(askOptions.signal, () => {
        void session?.abort?.();
      });
      if (askOptions.signal?.aborted) throw aiAbortError();
      await measureDebugPerformance(
        'ai.ask.prompt',
        {
          ...
          contextLength: askOptions.context?.length || 0,
      ...
      askOptions.onEvent?.({
        ...
        data: aiLimitNoticeFromError(error),
      });
      throw error;
    } finally {
      removeAbortListener();
  ...
  function stream(message: string, streamOptions: AiPromptOptions = {}) {
    ...
    const removeExternalAbortListener = bindAbortSignal(
  ...
  function session(
    ...
    return {
      ask: (message: string, options: AiPromptOptions = {}) =>
        ask(message, { ...sessionOptions, ...options, sessionId }),
      stream: (message: string, options: AiPromptOptions = {}) =>
        stream(message, { ...sessionOptions, ...options, sessionId }),
      reset: async () => {
  ...
  async function createSession(
    ...
    onEvent?.({
      type: 'debug',
      label: 'model-source',
      data: { source: modelSource, baseUrl: model.baseUrl },
    });
    const modelDebug = {
    ...
    const customTools = createTools(pi, ai.Type, {
      ...
      removeExtension: (filename) =>
        removeExtension?.(filename, chatId) || { removed: false },
      addAliasForChat,
      emitEvent: onEvent,
    });
      ...
      entry.unsubscribe = result.session.subscribe((event) => {
        ...
        if (isAssistantErrorEndEvent(event)) {
          ...
          emit({
            type: 'error',
            message,
            data: aiLimitNoticeFromError(message),
          });
        }
      });
...
function aiLimitNoticeFromError(error: unknown): AiLimitNotice | null {
  ...
  const retryAfterSec =
    Number(text.match(/retry[_ -]?after[^0-9]*(\d+)/i)?.[1] || 0) || undefined;
  ...
  if (
    /unsupported[_ -]?client|unsupported[_ -]?desktop|unsupported[_ -]?api|no longer supported|426\b/i.test(
      text,
    )
  ) {
    return {
...
function searchableErrorText(error: unknown) {
  ...
  return [error.name, error.message, error.stack, stringifyError(own)]
    .filter(Boolean)
    .join('\n');
}
  ...
  const result = (await pi.createAgentSession({
    ...
    sessionManager: pi.SessionManager.inMemory(
      path.join(
        workspaceDir,
        'extension-ai',
        sessionOptions.sessionId || `${Date.now()}-${Math.random()}`,
      ),
    ),
...
async function createResourceLoader(
  ...
  const extensionBuilderSkill = {
    ...
    description:
      'Builds local Nevermind extensions that return declarative UI views. Use whenever the user asks to automate or create a command/action in Nevermind.',
    filePath: skillPath,
    baseDir: path.dirname(skillPath),
    sourceInfo: pi.createSyntheticSourceInfo(skillPath, {
      source: 'nevermind',
    }),
  ...
  return {
    ...
    getSkills: () => {
      const webSkills = (webAccessLoader?.getSkills().skills || []).filter(
        (skill: { filePath?: string }) =>
          skill.filePath?.includes('pi-web-access'),
      );
    ...
    getAgentsFiles: () => ({
      agentsFiles: [
        { path: 'NEVERMIND.md', content: systemContext(extensionApiPath) },
      ],
    }),
...
function createTools(
  ...
  function observedTool<TParams>(
    name: string,
    execute: (toolCallId: string, params: TParams) => Promise<any>,
  ) {
    return async (toolCallId: string, params: TParams) => {
      emitEvent?.({
        type: 'tool_trace_start',
        name,
        data: { toolCallId, input: summarizedToolInput(params) },
      });
      try {
        ...
        emitEvent?.({
          type: 'tool_trace_end',
          name,
          data: { toolCallId, output: summarizedToolResult(result) },
          isError: false,
        });
        return result;
      } catch (error) {
  ...
  return [
    pi.defineTool({
      ...
      execute: observedTool('read_extension_api', async () => {
        if (cachedApiText === undefined)
          cachedApiText = await fs.readFile(extensionApiPath, 'utf8');
        return {
    ...
    pi.defineTool({
      ...
      description:
        'List UI views and OS capabilities available to generated Nevermind extensions.',
      parameters: Type.Object({}),
    ...
    pi.defineTool({
      ...
      description:
        'Read the current Nevermind keyboard shortcuts, including the app shortcut used to open Nevermind and active global action shortcuts.',
      parameters: Type.Object({}),
      execute: observedTool('list_shortcuts', async () => {
        ...
        return {
          content: [
            {
              type: 'text',
              text: JSON.stringify({ palette, shortcuts }, null, 2),
            },
          ],
    ...
    pi.defineTool({
      ...
      label: 'Read App Logs',
      description:
        'Read recent structured Nevermind app logs to debug host, renderer, extension, or API failures. Use this when an extension fails, an API helper behaves unexpectedly, or the user asks about app diagnostics.',
      parameters: Type.Object({
        limit: Type.Optional(
          Type.Number({
            description:
              'Maximum number of entries to return. Defaults to 200 and is capped by the host.',
          }),
        ),
        level: Type.Optional(
          Type.String({
            description: 'Optional level filter: debug, info, warn, or error.',
          }),
        ),
        source: Type.Optional(
          Type.String({
            description:
              'Optional source filter: main, renderer, extension, or host.',
          }),
        ),
        ...
        query: Type.Optional(
          Type.String({
            description:
              'Case-insensitive text filter across the structured log entry.',
          }),
        ),
        extensionId: Type.Optional(
          Type.String({ description: 'Optional extension id filter.' }),
        ),
      }),
    ...
    pi.defineTool({
      ...
      description:
        'Read the active generated extension source for this chat/action before tweaking it.',
      parameters: Type.Object({}),
        ...
        if (!filename)
          return {
            content: [
              {
                type: 'text',
                text: 'No focused generated extension for this chat.',
              },
            ],
            details: {},
          };
    ...
    pi.defineTool({
      ...
      description:
        'Write a generated Nevermind TypeScript extension into the generated extensions directory.',
      parameters: Type.Object({
        filename: Type.String({
          description: 'Safe filename ending in .ts, for example image-grid.ts',
        }),
        code: Type.String({
          description:
            'Complete TypeScript extension source code using export default { ... } satisfies NevermindExtension',
        }),
      }),
      execute: observedTool(
        'write_extension',
    ...
    pi.defineTool({
      ...
      description:
        'Remove a generated Nevermind TypeScript extension owned by this AI chat.',
      parameters: Type.Object({
        filename: Type.String({
          description: 'Safe generated extension filename ending in .ts',
        }),
      ...
      execute: observedTool(
        ...
        async (_toolCallId: string, params: { filename: string }) => {
          ...
          if (!canWriteExtension?.(filename))
            throw new Error(
              `Refusing to remove ${filename}: this AI chat does not own that extension.`,
            );
          const result = await removeExtension?.(filename);
          if (!result?.removed)
            return {
              content: [
                { type: 'text', text: `${filename} is already removed.` },
              ],
              details: { filePath },
            };
          return {
            content: [
              {
                type: 'text',
                text: `Removed ${filename} from ${result.filePath || filePath}`,
              },
            ],
    ...
    pi.defineTool({
      ...
      description:
        'Typecheck and runtime-validate a generated TypeScript extension file.',
      ...
      execute: observedTool(
        ...
        async (_toolCallId: string, params: { filename: string }) => {
          ...
          return {
            content: [
              { type: 'text', text: `Validated ${path.basename(filePath)}` },
            ],
            details: { filePath },
          };
        },
...
function summarizedToolInput(params: unknown) {
  ...
  if (typeof value.code === 'string')
    value.code = summarizeText(value.code, 500);
  if (typeof value.script === 'string')
    value.script = summarizeText(value.script, 500);
  return value;
}
...
function safeExtensionPath(root: string, filename: string) {
  ...
  if (!isExtensionSourceFile(safeName))
    throw new Error('Extension filename must end in .ts');
...
async function validateTypeScriptExtension(
  filePath: string,
  extensionTypesPath: string,
  extensionsDir: string,
) {
...
function capabilities() {
  return {
    ...
    itemOptions: [
      'accessories with tone/tooltip',
      'keywords',
      'detail markdown/metadata/actions',
      'image descriptor',
      'actionPanel',
      'appearance.foreground: muted named color yellow, blue, purple, green, red, orange, or pink applied to item/action/command title and Lucide/fallback icon',
    ],
    ...
    actions: [
      ...
      'openUrl',
      'copyText',
      'pasteText(options: keepPaletteOpen, restoreClipboard, plainText, concealed)',
      'paste(text/html/image/files content with concealed/restore options)',
      'typeText',
      'copyImage',
      'trash',
      'push',
    ...
    ai: [
      ...
      'attachments.text/image/file/selectedText/selectedFiles/clipboard/ocrImage',
    ],
    webTools: [
      'web_search',
      'code_search',
    ...
    desktop: {
      keyboard: [
        'typeText(text, options) types into the frontmost app without touching the clipboard when keyboard.type-text is available',
      ],
      clipboard: [
        'readText',
        'writeText(options.concealed)',
        'readHtml',
        'writeHtml',
        'readImage',
        'writeImage',
        ...
        'writeFiles',
        'read({formats})',
        'write(text/html/image/files content)',
      ],
    ...
    sortByDescriptions: {
      recent:
        'filesystem modification time (mtimeMs), not necessarily download/add time',
      modified: 'filesystem modification time (mtimeMs), same as recent',
      added:
        'Finder/Spotlight Date Added (dateAddedMs) with creation-time fallback; prefer for newest Downloads, screenshots, and mixed media galleries',
