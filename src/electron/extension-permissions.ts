...
export function filterWebviewPermissionsForExtension(extension: ExtensionLike | null | undefined, webviewPermissions: readonly string[] | undefined) {
  ...
  return Array.from(new Set(webviewPermissions)).filter((permission) => {
    ...
    return requiredPermissions.every((requiredPermission) => hasExtensionPermission(extension, requiredPermission))
  })
}
