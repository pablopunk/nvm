...
function createDeps(overrides: Partial<AppIpcHandlersDeps> = {}) {
  ...
  const deps: AppIpcHandlersDeps = {
    ...
    getDesignTokens: (event) => ({ event }),
    openDesignTokenEditor: (event) => ({ event }),
    setDesignTokens: (event, input) => ({ event, input }),
    resetDesignTokens: (event) => ({ event }),
    closeDesignTokenEditor: (event) => ({ event }),
