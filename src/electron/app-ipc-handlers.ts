import { createMeasuredIpcRegistrar, type IpcMainLike } from './ipc-registration'

export type AppIpcHandlersDeps = {
  ipcMain: IpcMainLike & { on(channel: string, handler: (...args: any[]) => unknown): void }
  measureDebugPerformance: <T>(name: string, data: Record<string, unknown>, fn: () => T | Promise<T>) => T | Promise<T>
  ...
  userDataPath: () => string
  signInToNevermind: () => Promise<{ ok: true; auth: { baseUrl: string; email?: string } } | { ok: false; error?: string }>
  invalidateExtensionRootItems: () => unknown
  broadcastAuthChanged: (status: { authed: boolean; email?: string }) => unknown
  appIconCache: { get(appPath: unknown): unknown }
  runningAppStatus: { getForRenderer(appPaths: unknown): unknown }
  paletteWindow: { setPaletteSizeForMode(mode: unknown): unknown; centerWindow(): unknown; hidePalette(): unknown; revealPalette(): unknown }
  ...
  BrowserWindow: { fromWebContents(sender: unknown): { close(): void } | null | undefined }
...
export function registerAppIpcHandlers(deps: AppIpcHandlersDeps) {
  const ipcHandleMeasured = createMeasuredIpcRegistrar({ ipcMain: deps.ipcMain, measure: deps.measureDebugPerformance, summarize: deps.summarizeDebugValue })

  ipcHandleMeasured('actions:search', (_event, query, options) => deps.searchActions(query, options))
  ipcHandleMeasured('actions:execute', (_event, action) => deps.executeActionForIpc(action))
  ipcHandleMeasured('view-action:execute', (_event, action) => deps.executeViewActionForIpc(action))
  deps.ipcMain.handle('view:refresh', (event, input) => deps.measureDebugPerformance('ipc.view:refresh.handler', { args: [deps.summarizeDebugValue(input)], alwaysLog: true }, () => deps.refreshViewForIpc(input)))
  ipcHandleMeasured('dialog:pick-form-field-paths', deps.pickFormFieldPaths)
  deps.ipcMain.on('drag:file', deps.startFileDrag)
  ipcHandleMeasured('ai:chat:send', (_event, message, chatId) => deps.sendAiChatMessage(message, chatId))
  ipcHandleMeasured('ai:chat:exited', (_event, chatId) => deps.noteAiChatExited(chatId))
  ipcHandleMeasured('ai:chat:abort', (_event, chatId) => deps.abortAiChat(chatId))
  ipcHandleMeasured('ai:chat:reset', (_event, chatId) => deps.resetAiChat(chatId))
  ipcHandleMeasured('actions:set-alias', (_event, action, alias) => deps.setAlias(action, alias))
  ipcHandleMeasured('actions:remove-alias', (_event, action, alias) => deps.removeAlias(action, alias))
  ipcHandleMeasured('actions:set-shortcut', (_event, action, shortcut) => deps.setShortcut(action, shortcut))
  ipcHandleMeasured('palette:set-hotkey', (_event, accelerator) => deps.setPaletteHotkey(accelerator))
  ipcHandleMeasured('settings:get', (_event, id) => deps.getSetting(id))
  ipcHandleMeasured('system:open-keyboard-settings', () => deps.openSystemKeyboardSettings())
  ipcHandleMeasured('actions:get-shortcuts', () => deps.getShortcuts())
  ipcHandleMeasured('actions:remove-shortcut', (_event, actionId) => deps.removeShortcut(actionId))
  ipcHandleMeasured('actions:suspend-shortcuts', () => deps.unregisterActionShortcuts())
  ipcHandleMeasured('actions:resume-shortcuts', () => deps.registerActionShortcuts())
  ipcHandleMeasured('actions:set-override', (_event, action, instruction) => deps.setOverride(action, instruction))
  ipcHandleMeasured('actions:clear-override', (_event, action) => deps.clearOverride(action))
  ipcHandleMeasured('actions:duplicate-created', (_event, action) => deps.duplicateCreatedAction(action))
  ipcHandleMeasured('actions:remove-created', (_event, action) => deps.removeCreatedAction(action))
  ipcHandleMeasured('ai-builder:tweak-extension', (_event, input: any = {}) => {
    ...
    return deps.normalizeHostViewResult({ view: deps.aiChatView(item, { initialPrompt: input.prompt }) })
  })
  ipcHandleMeasured('ai-builder:start-chat', (_event, input: any = {}) => {
    const item = deps.createDraftAiChat(String(input?.prompt || input?.query || ''))
    return deps.normalizeHostViewResult({ view: deps.aiChatView(item, { start: (item as any).messages.length <= 1 }) })
  })
  ...
  ipcHandleMeasured('palette:shortcut-ready', () => deps.paletteWindow.revealPalette())
  ipcHandleMeasured('camera:request-access', async () => {
    if (!deps.hasCapability('camera')) return { ok: false, status: 'unsupported' }
    if (deps.processPlatform !== 'darwin') return { ok: true, status: 'unknown' }
  ...
  ipcHandleMeasured('extension-window:get-state', (_event, id) => deps.extensionWindowManager.getState(String(id || '')))
  ...
  ipcHandleMeasured('logs:write', (_event, level, message, data) => {
    const method = level === 'error' ? deps.logError : level === 'warn' ? deps.logWarn : level === 'debug' ? deps.loggerDebug : deps.logInfo
