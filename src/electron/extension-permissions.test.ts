...
test('only host-marked internal extensions are trusted when they omit explicit permissions', () => {
  ...
  assert.equal(
    hasExtensionPermission({ id: 'nevermind.spoof' }, 'system'),
    false,
  );
});
...
test('explicit permission arrays are authoritative even for internal extensions', () => {
  assert.equal(
    hasExtensionPermission(
      markInternalExtension({ id: 'nevermind.fixture', permissions: [] }),
      'system',
    ),
    false,
  );
  assert.equal(
    hasExtensionPermission({ id: 'third.party', permissions: ['ai'] }, 'ai'),
    true,
  );
  assert.equal(
    hasExtensionPermission(
      { id: 'third.party', permissions: ['ai'] },
      'system',
    ),
    false,
  );
...
test('filterWebviewPermissionsForExtension gates delegated iframe permissions by host manifest permissions', () => {
  assert.deepEqual(
    filterWebviewPermissionsForExtension(
      { id: 'third.party', permissions: ['camera', 'clipboard.history'] },
      [
        'autoplay',
        'camera',
        'microphone',
    ...
    ['autoplay', 'camera', 'microphone', 'clipboard-read', 'clipboard-write'],
  );
  assert.deepEqual(
    filterWebviewPermissionsForExtension(
  ...
  assert.deepEqual(
    filterWebviewPermissionsForExtension(
      { id: 'third.party', permissions: ['desktop.files'] },
      ['display-capture'],
    ),
    ['display-capture'],
  );
...
test('permissionDeniedError uses stable host error text', () => {
  assert.equal(
    permissionDeniedError('system').message,
    'Extension is missing required permission: system',
  );
});
