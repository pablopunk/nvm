...
export function onNevermindCompatibilityChanged(listener: CompatibilityListener) {
  listeners.add(listener)
  return () => listeners.delete(listener)
}

export function currentNevermindCompatibilityManifest(baseUrl?: string) {
  if (baseUrl) return cachedManifests.get(normalizeBaseUrl(baseUrl))?.manifest || null
  return [...cachedManifests.values()]
...
export function warmNevermindCompatibilityCache(baseUrl: string) {
  ...
  })().catch((error) => logger.warn('nevermind.compatibility.warm.failed', error as Error))
}
...
export async function checkNevermindCompatibility(baseUrl: string) {
  ...
  if (effectiveManifest.client?.compatible === false) throw new NevermindCompatibilityError(effectiveManifest)
...
async function cacheCompatibilityManifest(baseUrl: string, manifest: NevermindCompatibilityManifest) {
  await loadCompatibilityCache()
  cachedManifests.set(baseUrl, { baseUrl, fetchedAt: new Date().toISOString(), manifest })
...
async function loadCompatibilityCache() {
  ...
  cacheLoadPromise = (async () => {
    try {
      const data = JSON.parse(await fs.readFile(compatibilityCachePath(), 'utf8')) as CompatibilityCacheFile
      cachedManifests.clear()
      for (const [baseUrl, entry] of Object.entries(data.manifests || {})) {
        if (entry?.manifest) cachedManifests.set(normalizeBaseUrl(baseUrl), { ...entry, baseUrl: normalizeBaseUrl(entry.baseUrl || baseUrl) })
      }
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code !== 'ENOENT') logger.warn('nevermind.compatibility.cache.read.failed', error as Error)
    }
  })()
...
async function saveCompatibilityCache() {
  try {
    await fs.writeFile(compatibilityCachePath(), JSON.stringify({ manifests: Object.fromEntries(cachedManifests) }, null, 2), { mode: 0o600 })
  } catch (error) {
