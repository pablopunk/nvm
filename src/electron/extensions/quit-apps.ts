    ...
    return [
      {
        ...
        run: async (ctx) => {
          return ctx.ui.confirm({
            title: 'Quit All Apps?',
            message: 'This will close all visible applications. Unsaved changes may be lost.',
            ...
            onConfirm: ctx.actions.run('Quitting Apps', async (ctx) => {
              ...
              try {
                ...
                ctx.logs.error('Failed to execute Quit Apps script', { error: err.message })
                return ctx.ui.toast({ message: `Error: ${err.message}`, tone: 'error' })
              }
            }),
          })
        },
