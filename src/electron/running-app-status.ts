...
type MeasureAsync = <T>(name: string, metadata: Record<string, unknown>, work: () => Promise<T>) => Promise<T>
...
export type RunningAppStatusServiceOptions = {
  ...
  detectRunningAppPaths: (candidates: RunningAppCandidate[]) => Promise<Set<string>>
  notifyChanged: () => void
...
function normalizedRunningPath(value: unknown, platform: NodeJS.Platform) {
  ...
  return platform === 'darwin' || platform === 'win32' ? text.toLowerCase() : text
}
...
export function createRunningAppStatusService(options: RunningAppStatusServiceOptions): RunningAppStatusService {
  ...
  async function getForRenderer(appPaths: unknown) {
    return measure('apps.running.get', { requestedCount: Array.isArray(appPaths) ? appPaths.length : 0, cached: Boolean(snapshot), alwaysLog: true }, async () => {
      ...
      return requestedPaths.filter((appPath) => runningPaths.has(normalizedRunningPath(appPath, platform)))
    })
  }
