...
function createCache(options: { hasIcons?: boolean; cached?: Buffer | null; icon?: Buffer } = {}) {
  ...
  const cache = createAppIconCache({
    ...
    writeCachedIcon: async (key, png) => { written.push({ key, png }) },
...
test('app icon cache dedupes concurrent requests and resolves waiters from one load', async () => {
  ...
  assert.deepEqual(written.map((item) => item.key), ['hash:/Applications/TextEdit.app'])
})

test('app icon cache uses disk cache before loading native app icon', async () => {
  const { cache, loaded, marks } = createCache({ cached: Buffer.from('cached') })

  assert.equal(await cache.loadAppIconDataUrl('/Applications/TextEdit.app'), 'data:image/png;base64,Y2FjaGVk')
...
test('app icon cache schedules backlog batches when pending work remains', async () => {
  ...
  const requests = Array.from({ length: 21 }, (_, index) => cache.get(`/Applications/App${index}.app`))
  await cache.processPending()
