...
export function createDataLoaderHandle(fn: () => Promise<any[]>, options: { retry?: boolean } = {}): DataLoaderHandle {
...
export function resolveLoaderEmptyView(emptyView: any, loaderHandle?: DataLoaderHandle) {
...
export function createViewLoaderRegistry(deps: {
  ...
  async function spawn(viewId: string) {
    ...
    try {
      ...
      deps.sendHydrate(viewId, { items: Array.isArray(items) ? deps.normalizeItems(items, loader.entry) : [], isLoading: false })
