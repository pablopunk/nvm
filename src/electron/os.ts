...
export function hasCapability(capability: string) {
  if (capability === 'ocr') return osDependent({ darwin: fsSync.existsSync('/usr/bin/swift') }, false)
  if (capability === 'screen-capture') return osDependent({ darwin: fsSync.existsSync('/usr/sbin/screencapture') }, false)
...
function execFileJson(command: string, args: string[], options: any = {}) {
  return new Promise<any>((resolve, reject) => {
    execFile(command, args, { timeout: options.timeout || 30_000, maxBuffer: options.maxBuffer || 2 * 1024 * 1024 }, (error, stdout, stderr) => {
      if (error) return reject(new Error(String(stderr || error.message || 'Command failed').trim()))
      try { resolve(JSON.parse(stdout || '{}')) } catch { reject(new Error('OCR returned invalid JSON')) }
    })
  })
}
...
async function recognizeTextInImageMac(filePath: string, options: any = {}) {
  ...
  const languages = Array.isArray(options.languages) ? options.languages.map(String) : options.language ? [String(options.language)] : []
  return execFileJson('/usr/bin/swift', [helperPath, filePath, ...languages], { timeout: Math.max(5_000, Number(options.timeoutMs || 30_000)) })
}

export async function recognizeTextInImage(filePath: string, options: any = {}) {
  return osFunction<[string, any], Promise<any>>({
    darwin: recognizeTextInImageMac,
  }, async () => { throw new Error(`OCR is not available on ${osLabel()}`) })(filePath, options)
}
...
async function captureScreenImageMac(options: any = {}) {
  ...
  const args = ['-x']
  if (rect && Number(rect.width) > 0 && Number(rect.height) > 0) args.push('-R', [rect.x || 0, rect.y || 0, rect.width, rect.height].map((value) => Math.round(Number(value))).join(','))
  args.push(outputPath)
  await new Promise<void>((resolve, reject) => {
    execFile('/usr/sbin/screencapture', args, { timeout: Math.max(5_000, Number(options.timeoutMs || 15_000)) }, (error, _stdout, stderr) => error ? reject(new Error(String(stderr || error.message || 'Screen capture failed').trim())) : resolve())
  })
  return outputPath
}
...
export async function captureScreenImage(options: any = {}) {
  ...
  }, async () => { throw new Error(`Screen capture is not available on ${osLabel()}`) })(options)
}
