...
import {
  createRunningAppStatusService,
  type RunningAppCandidate,
} from './running-app-status';
...
test('returns cached running paths immediately while refreshing stale snapshots', async () => {
  ...
  let detectCount = 0;
  let changeCount = 0;
  const candidates: RunningAppCandidate[] = [
  ...
  secondRefresh.resolve(
    new Set(['/applications/notes.app', '/applications/mail.app']),
  );
...
test('normalizes detected running paths before storing snapshots', async () => {
  ...
  assert.deepEqual(await service.getForRenderer(['/Applications/Notes.app']), [
...
test('dedupes in-flight refreshes and caps renderer requested paths', async () => {
  ...
  const requested = Array.from({ length: 35 }, (_item, index) =>
    index === 0 ? '/usr/bin/app' : `/tmp/${index}`,
  );
