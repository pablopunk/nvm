...
export function debug(message: string, data?: unknown, meta?: Partial<LogEntry>) {
  write('debug', message, data, meta)
}

export function info(message: string, data?: unknown, meta?: Partial<LogEntry>) {
  write('info', message, data, meta)
}

export function warn(message: string, data?: unknown, meta?: Partial<LogEntry>) {
  write('warn', message, data, meta)
}

export function error(message: string, data?: unknown, meta?: Partial<LogEntry>) {
...
export async function readRecentLogs(options: RecentLogOptions = {}) {
  const limit = Math.min(Math.max(Number(options.limit || DEFAULT_RECENT_LIMIT), 1), MAX_RECENT_LIMIT)
  ...
  const entries = text
    ...
    .filter((entry) => !options.extensionId || entry.extensionId === options.extensionId || lineContains(entry, options.extensionId))
    .filter((entry) => !since || Date.parse(entry.timestamp) >= since)
    .filter((entry) => !query || lineContains(entry, query))
  return entries.slice(-limit)
}

function write(level: LogLevel, message: string, data?: unknown, meta: Partial<LogEntry> = {}) {
...
function writePrettyConsole(entry: LogEntry) {
  ...
  const scope = [entry.source, entry.scope, entry.extensionId, entry.commandId].filter(Boolean).join(':')
...
function parseLine(line: string) {
  ...
  try {
    ...
    if (!value || typeof value !== 'object' || !value.timestamp || !value.level || !value.message) return null
...
function lineContains(entry: LogEntry, query: string) {
  return JSON.stringify(entry).toLowerCase().includes(String(query).toLowerCase())
}
