...
test('handles local date and timezone questions', () => {
  ...
  assert.equal(
    calculateDetailed('days until 25 Dec', { now })?.raw,
    '202 days',
  );
  ...
  assert.equal(
    calculateDetailed('Monday in 3 weeks', { now })?.raw,
    '2026-06-29',
  );
...
test('parses and formats cached fiat rates', () => {
  ...
  assert.deepEqual(
    usdToGbp && {
      ...
      sourceCurrency: usdToGbp.sourceCurrency,
      targetCurrency: usdToGbp.targetCurrency,
    },
    { amount: 10, sourceCurrency: 'USD', targetCurrency: 'GBP' },
  );
  assert.equal(
    calculateRateResult('10 usd in gbp', usdToGbp!, quote)?.raw,
    '8 GBP',
  );
  ...
  assert.deepEqual(
    prefixed && {
      amount: prefixed.amount,
      sourceCurrency: prefixed.sourceCurrency,
      targetCurrency: prefixed.targetCurrency,
    },
    { amount: 1_200_000, sourceCurrency: 'USD', targetCurrency: 'GBP' },
  );
  assert.equal(
    calculateRateResult('$1.2m in gbp', prefixed!, quote)?.raw,
    '960000 GBP',
  );
  ...
  assert.deepEqual(
    hourly && {
      ...
      sourceCurrency: hourly.sourceCurrency,
      targetCurrency: hourly.targetCurrency,
      rateUnit: hourly.rateUnit,
    },
    {
      amount: 8,
      sourceCurrency: 'USD',
  ...
  assert.equal(
    calculateRateResult('8 dollars/hour in gbp', hourly!, quote)?.raw,
    '6.4 GBP/hour',
  );

  assert.deepEqual(
    parseRateExpression('10 $') && {
      amount: parseRateExpression('10 $')!.amount,
      sourceCurrency: parseRateExpression('10 $')!.sourceCurrency,
      targetCurrency: parseRateExpression('10 $')!.targetCurrency,
    },
  ...
  assert.deepEqual(
    ...
    { amount: 10, sourceCurrency: 'EUR', targetCurrency: 'USD' },
  );
  assert.deepEqual(
    parseRateExpression('€10') && {
      amount: parseRateExpression('€10')!.amount,
