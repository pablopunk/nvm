...
const INTERNAL_EXTENSION_FACTORIES: Array<() => any> = [
  ...
  ai.attachments = {
    ...
    clipboard: async (options: any = {}) => {
      if (!capabilities.clipboard)
        throw permissionDeniedError('clipboard.history');
...
function createSystemExtension() {
  ...
  const commands = systemItems(createExtensionContext(extension, null)).map(
    commandFromItem,
  );
  return {
    ...extension,
      ...
      return [
        {
          ...
          icon: 'search',
          score:
            10 + usageBoost(`web-search:${q}`) + recentBoost(`web-search:${q}`),
          dismissAfterRun: 'auto',
          primaryAction: {
            type: 'openUrl',
...
function appRootItem(item) {
  ...
  return {
    ...
    aliases: actionAliases(`extension-root:nevermind.apps:${id}`),
    icon: 'app',
...
function fileRootItem(item) {
  ...
  const previewAction = buildPreviewItemAction({
    kind: item.kind || 'file',
    title: 'Preview',
    filePath: item.path,
  ...
  return {
    ...
    rootItems(ctx) {
      return ctx.desktop.files
        .recent({ limit: FILE_RESULT_LIMIT })
        .map(fileRootItem);
    },
    searchItems(ctx, query) {
      return ctx.desktop.files
...
function createUpdatesExtension() {
  ...
  return {
    ...
    rootItems() {
      return [
        compatibilityPromptAction() || updatePromptAction() || checkItem(),
      ];
    },
  };
}
