...
export function debugPerformanceEnabled() {
  ...
  return Boolean(process.env.NVM_DEBUG_PERFORMANCE || process.env.NVM_PALETTE_DEBUG || process.env.ELECTRON_RENDERER_URL)
}

export function markDebugPerformance(name: string, detail?: DebugPerformanceDetail) {
...
export async function measureDebugPerformance<T>(name: string, detail: DebugPerformanceDetail, task: () => Promise<T> | T) {
...
export function measureDebugPerformanceSync<T>(name: string, detail: DebugPerformanceDetail, task: () => T) {
  if (!debugPerformanceEnabled()) return task()
  const measurement = startDebugPerformanceMeasure(name, detail)
  try {
    return task()
...
export function summarizeDebugValue(value: unknown): unknown {
  ...
  if (typeof value === 'string') return value.length > 80 ? { type: 'string', length: value.length, preview: value.slice(0, 80) } : value
  if (typeof value === 'number' || typeof value === 'boolean') return value
  if (Array.isArray(value)) return { type: 'array', length: value.length }
  if (typeof value === 'object') {
    const record = value as Record<string, unknown>
    const summary: Record<string, unknown> = { type: 'object' }
    for (const key of ['id', 'kind', 'type', 'title', 'extensionId', 'commandId', 'handlerId', 'viewId']) {
...
function startDebugPerformanceMeasure(name: string, detail?: DebugPerformanceDetail) {
...
function finishDebugPerformanceMeasure(measurement: { name: string; detail?: DebugPerformanceDetail; startMark: string; startedAt: number }) {
  ...
  try {
    performance.measure(`nvm:${measurement.name}`, measurement.startMark, endMark)
  } catch {}
...
function logSlowDebugPerformance(name: string, durationMs: number, detail?: DebugPerformanceDetail) {
  const thresholdMs = Number(process.env.NVM_DEBUG_PERFORMANCE_SLOW_MS || DEFAULT_SLOW_LOG_THRESHOLD_MS)
  if (durationMs < thresholdMs && !detail?.alwaysLog) return
  logDebug('performance.measure', {
    name,
