...
function createUnitMap() {
  ...
  const addTemperature = (
    ...
    addUnit(
      units,
      { dimension: 'temperature', symbol, toBase, fromBase },
      aliases,
    );
  };
  ...
  addLinear('volume', 'tsp', 0.00492892159375, [
    'tsp',
    'teaspoon',
    'teaspoons',
  ]);
  addLinear('volume', 'tbsp', 0.01478676478125, [
    'tbsp',
    'tablespoon',
    'tablespoons',
  ]);
...
function createCurrencyMap() {
  ...
  const add = (code: string, aliases: string[]) => {
    ...
    for (const alias of aliases)
      currencies.set(normalizeCurrencyAlias(alias), code);
  };
...
export function calculateDetailed(
  ...
  if (
    !normalized ||
    !isLikelyCalculation(normalized.expression, normalized.explicit)
  )
    return null;

  const dateTimeResult = calculateDateTimeExpression(
  ...
  const conversionResult = calculateConversionExpression(
    normalized.expression,
    query,
  );
  if (conversionResult) return conversionResult;

  const unitPercentResult = calculateUnitPercentExpression(
    normalized.expression,
    query,
  );
...
function normalizeCalculationQuery(query: string) {
  ...
  return {
    expression: withoutCalc.trim(),
    explicit: withoutEquals !== trimmed || withoutCalc !== withoutEquals,
  };
}
...
function looksLikeConversionExpression(expression: string) {
  return (
    conversionExpressionMatch(expression) !== null ||
    parseRateExpression(expression) !== null
  );
}

function looksLikeDateTimeExpression(expression: string) {
  return /^(?:(?:days?\s+until\s+)?(?:\d{1,2}\s+[a-z]+|[a-z]+\s+\d{1,2})|\d+\s+days?\s+ago|(?:next\s+)?(?:sun|mon|tue|wed|thu|fri|sat|sunday|monday|tuesday|wednesday|thursday|friday|saturday)(?:\s+in\s+\d+\s+weeks?)?|tomorrow|yesterday|time\s+in\s+.+|\d{1,2}(?::\d{2})?\s*(?:am|pm)?\s+.+\s+in\s+.+)$/i.test(
    expression.trim(),
  );
}

function calculateDateTimeExpression(
  expression: string,
  query: string,
  options: CalculatorOptions,
): CalculatorResult | null {
  return (
    calculateDateExpression(expression, query, options) ||
    calculateTimezoneExpression(expression, query, options)
  );
}

function calculateDateExpression(
  expression: string,
  query: string,
  options: CalculatorOptions,
): CalculatorResult | null {
  ...
  if (monthDay) {
    ...
    if (monthDay.until) {
      ...
      return dateResult(
        ...
        text,
        text,
        `Until ${formatDateDisplay(target)}`,
      );
    }
    return dateResult(
      'date',
...
function calculateTimezoneExpression(
  expression: string,
  query: string,
  options: CalculatorOptions,
): CalculatorResult | null {
  ...
  const conversion = expression
    .trim()
    .match(/^(\d{1,2})(?::(\d{2}))?\s*(am|pm)?\s+(.+?)\s+in\s+(.+)$/i);
  ...
  const instant = instantForZonedTime(sourceZone, {
...
function timezoneResult(
  ...
  return {
    ...
    timezone: zone,
    title: `${expression.trim()} = ${formatted}`,
    subtitle: `Time in ${zone}`,
  };
}

export function calculateRateResult(
  query: string,
  parsed: RateExpression,
  quote: RateQuote,
): CalculatorResult | null {
  ...
  const raw = parsed.rateUnit
    ? `${rawAmount} ${parsed.targetCurrency}/${parsed.rateUnit}`
    : `${rawAmount} ${parsed.targetCurrency}`;
  const formatted = parsed.rateUnit
    ? `${formattedAmount}/${parsed.rateUnit}`
    : formattedAmount;
...
export function parseRateExpression(query: string): RateExpression | null {
  ...
  const suffixMatch = expression
    .trim()
    .match(
      new RegExp(
        `^${amount}\\s*${currency}${rateUnit}\\s+(?:in|to|as)\\s+${currency}$`,
        'i',
      ),
    );
  const prefixMatch = expression
    .trim()
    .match(
      new RegExp(
        `^${currency}\\s*${amount}${rateUnit}\\s+(?:in|to|as)\\s+${currency}$`,
        'i',
      ),
    );
  const defaultSuffixMatch = expression
    .trim()
    .match(new RegExp(`^${amount}\\s*([$€])$`, 'i'));
  const defaultPrefixMatch = expression
    .trim()
    .match(new RegExp(`^([$€])\\s*${amount}$`, 'i'));
  const sourceAlias =
    suffixMatch?.[2] ||
    prefixMatch?.[1] ||
    defaultSuffixMatch?.[2] ||
    defaultPrefixMatch?.[1];
  const amountValue =
    suffixMatch?.[1] ||
    prefixMatch?.[2] ||
    defaultSuffixMatch?.[1] ||
    defaultPrefixMatch?.[2];
  ...
  const targetCurrency = targetAlias
    ? currencyForAlias(targetAlias)
    : defaultTargetCurrencyFor(sourceCurrency);
  if (
    !sourceCurrency ||
    !targetCurrency ||
...
function calculateConversionExpression(
  expression: string,
  query: string,
): CalculatorResult | null {
  ...
  if (!isTimespan && (!target || source.dimension !== target.dimension))
    return null;
  ...
  const raw = isTimespan
    ? formatTimespan(value)
    : `${formatRawNumber(value, 4)} ${target!.symbol}`;
  const formatted = isTimespan
    ? raw
    : `${formatDisplayNumber(value, 4)} ${target!.symbol}`;
  const swapQuery = isTimespan
    ? undefined
    : `${formatRawNumber(value, 4)} ${target!.symbol} to ${source.symbol}`;
  return {
    kind: 'conversion',
    query: query.trim(),
    expression: expression.trim(),
...
function conversionExpressionMatch(expression: string) {
  ...
  const match = expression
    ...
    .match(
      new RegExp(
        `^(?:convert\\s+)?(${number})\\s*(${unit})\\s+(?:in|to|as)\\s+(${unit}|timespan)$`,
        'i',
      ),
    );
  if (!match) return null;
...
function parseMonthDayExpression(expression: string) {
  ...
  if (dayFirst)
    return {
      until: Boolean(dayFirst[1]),
      day: Number(dayFirst[2]),
      month: dayFirst[3],
    };
  const monthFirst = trimmed.match(
    /^(days?\s+until\s+)?([a-z]+)\s+(\d{1,2})$/i,
  );
  if (monthFirst)
    return {
      until: Boolean(monthFirst[1]),
...
function instantForZonedTime(timeZone: string, target: ZonedParts) {
  ...
  let candidate = new Date(
    localTimestamp - timezoneOffsetMs(new Date(localTimestamp), timeZone),
  );
...
function formatCurrencyDisplay(value: number, currency: string) {
  try {
    ...
    }).format(normalizeFloatingPoint(value));
  } catch {
...
function calculateUnitPercentExpression(
  expression: string,
  query: string,
): CalculatorResult | null {
  const match = expression
    .trim()
    .match(
      /^([+-]?(?:\d+(?:,\d{3})*|\d*)(?:\.\d+)?)\s*([a-zA-Z]+)\s*([+-])\s*([+-]?(?:\d+(?:,\d{3})*|\d*)(?:\.\d+)?)\s*%$/,
    );
  if (!match) return null;
...
function tokenize(expression: string): Token[] {
  ...
  while (index < expression.length) {
    ...
    if (/[a-zA-Zπ]/.test(character)) {
      const start = index;
      while (index < expression.length && /[a-zA-Zπ]/.test(expression[index]))
        index += 1;
      tokens.push({
        type: 'identifier',
...
function isNumberStart(expression: string, index: number) {
  return (
    ...
    (expression[index] === '.' && /\d/.test(expression[index + 1] || ''))
  );
}
  ...
  private parseExpression(minPrecedence: number): ExpressionNode {
    ...
    while (true) {
      ...
      const right = this.parseExpression(
        operatorPrecedence + (operator === '^' ? 0 : 1),
      );
