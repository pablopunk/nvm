...
test('ai chat events are isolated to the active extension-window chat id', () => {
  assert.equal(aiChatEventMatchesActiveChat({ type: 'delta', chatId: 'chat-a', text: 'A' }, 'chat-a'), true)
  assert.equal(aiChatEventMatchesActiveChat({ type: 'delta', chatId: 'chat-b', text: 'B' }, 'chat-a'), false)
  assert.equal(aiChatEventMatchesActiveChat({ type: 'delta', chatId: 'chat-b', text: 'B' }, undefined), false)
})
