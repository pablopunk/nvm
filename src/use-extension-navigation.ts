...
export function nextNavigationState(current: NavigationState, nextView: CommandView, navigation: Exclude<NavigationMode, 'pop'> = 'replace'): NavigationState {
  if (navigation === 'root') return { view: nextView, backStack: [] }
  if (navigation === 'push' && current.view) return { view: nextView, backStack: [...current.backStack, current.view] }
  return { view: nextView, backStack: current.backStack }
}

export function previousNavigationState(current: NavigationState): { state: NavigationState; didPop: boolean } {
  const next = [...current.backStack]
  const previous = next.pop() || null
  return { state: { view: previous, backStack: next }, didPop: Boolean(previous) }
}

export function useExtensionNavigation() {
  const [state, setState] = useState<NavigationState>({ view: null, backStack: [] })
  ...
  function setBackStack(nextBackStack: SetStateAction<CommandView[]>) {
    setState((current) => ({
      ...current,
      backStack: typeof nextBackStack === 'function' ? nextBackStack(current.backStack) : nextBackStack,
    }))
  }
