...
export async function measureDebugPerformance<T>(name: string, detail: DebugPerformanceDetail, task: () => Promise<T>) {
...
function logSlowDebugPerformance(name: string, durationMs: number, detail?: DebugPerformanceDetail) {
  const thresholdMs = Number(window.localStorage?.getItem('nvm.debugPerformance.slowMs') || DEFAULT_SLOW_LOG_THRESHOLD_MS)
  if (durationMs < thresholdMs && !detail?.alwaysLog) return
  queueMicrotask(() => {
    window.nvm?.log?.('debug', 'performance.measure', {
      name,
