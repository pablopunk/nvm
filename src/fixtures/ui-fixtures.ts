  ...
  return ctx.ui.list({
    ...
    items: [
      ...
      ctx.ui.item({
        ...
        detail: { title: 'Warning Accessory', markdown: '`appearance.foreground` colors both the row title and the Lucide icon while accessories keep their own tones.', metadata: [{ type: 'tag', label: 'Tone', value: 'warning', tone: 'warning' }] },
      }),
      ctx.ui.item({
        id: 'danger',
        ...
        detail: { title: 'Danger Accessory', markdown: 'Foreground styling uses existing semantic color tokens and remains compact.' },
      }),
    ],
  })
}
