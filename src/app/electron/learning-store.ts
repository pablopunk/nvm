...
function normalizedExtensionFiles(extensionFiles: unknown) {
  return Array.from(
    new Set(
      (Array.isArray(extensionFiles) ? extensionFiles : [])
...
export class LocalLearningStore {
  ...
  async load() {
    ...
    try {
      const parsedTraces = tracesText
        ? (JSON.parse(tracesText) as Partial<
            Pick<LearningStoreState, 'traces'>
          >)
        : null;
      const parsedLearnings = parseLearningsContent(learningsText);
      this.state = {
        version: 1,
        traces: Array.isArray(parsedTraces?.traces)
          ? parsedTraces.traces.map((trace) => this.normalizedTrace(trace))
          : [],
        learnings: Array.isArray(parsedLearnings?.learnings)
          ? parsedLearnings.learnings.map((learning) =>
              this.normalizedLearning(learning),
            )
          : [],
      };
  ...
  appendAssistantDelta(
    ...
    if (last?.role === 'assistant')
      last.content = `${last.content}${delta}`.slice(0, MAX_MESSAGE_LENGTH);
    else
      trace.messages.push({
        role: 'assistant',
        content: delta,
  ...
  recordToolStart(
    ...
    const trace = this.ensureTrace(chatId, metadata);
    const toolCallId =
      preferredId || `${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
    trace.toolCalls.push({
      id: toolCallId,
      name: String(name || 'tool'),
      startedAt: Date.now(),
      inputSummary: sanitizedData(inputSummary),
    });
  ...
  recordToolEnd(
    ...
    const toolCall =
      trace.toolCalls.find((item) => item.id === toolCallId) ||
      lastMatchingToolCall(trace.toolCalls, name);
    if (!toolCall) return;
    toolCall.endedAt = Date.now();
    toolCall.ok = detail.ok ?? !detail.error;
    toolCall.outputSummary = sanitizedData(detail.outputSummary);
    toolCall.error = detail.error
      ? limitedText(detail.error, 2_000)
      : undefined;
    trace.updatedAt = Date.now();
  ...
  recordExtensionEvent(
    chatId: string,
    event: Omit<LearningTraceExtensionEvent, 'createdAt'>,
    metadata: TraceMetadata = {},
  ) {
    const trace = this.ensureTrace(chatId, metadata);
    trace.extensionEvents.push({
      ...
      commandIds: Array.isArray(event.commandIds)
        ? event.commandIds.filter(Boolean).map(String)
        : undefined,
      createdAt: Date.now(),
      details: sanitizedData(event.details),
    });
    trace.extensionEvents = trace.extensionEvents.slice(
      -MAX_TRACE_EXTENSION_EVENTS,
    );
    trace.updatedAt = Date.now();
    this.scheduleSave();
  }
  ...
  recordStatus(
    chatId: string,
    status: LearningTraceStatusEvent['status'],
    metadata: TraceMetadata = {},
    error?: string,
  ) {
    const trace = this.ensureTrace(chatId, metadata);
    trace.statusEvents.push({
      status,
      createdAt: Date.now(),
      error: error ? limitedText(error, 2_000) : undefined,
    });
  ...
  shouldReview(chatId: string) {
    ...
    return (
      ...
      trace.messages.length > 2 ||
      trace.extensionEvents.length > 0
    );
  }
  ...
  replaceLearningsFromReview(
    chatId: string,
    reviewLearnings: ReviewLearningInput[],
  ) {
    const trace = this.state.traces.find((item) => item.chatId === chatId);
    if (!trace) return [];
    const now = Date.now();
    ...
    for (const item of reviewLearnings) {
      ...
      if (!summary) continue;
      const kind =
        item.kind === 'workflow' || item.kind === 'preference'
          ? item.kind
          : 'environment';
      if (!qualifiesUserLearning({ ...item, kind, summary })) continue;
      const fingerprint = learningFingerprint(kind, summary);
      const existing = this.state.learnings.find(
        (learning) => learning.fingerprint === fingerprint,
      );
      next.push({
        ...
        appliesWhen: item.appliesWhen
          ? limitedText(item.appliesWhen, 300)
          : undefined,
        ...
        evidenceChatIds: Array.from(
          new Set([...(existing?.evidenceChatIds || []), chatId]),
        ),
        extensionFiles: Array.from(
          new Set([
            ...(existing?.extensionFiles || []),
            ...trace.extensionFiles,
          ]),
        ),
        toolNames: Array.from(
          new Set([
            ...(existing?.toolNames || []),
            ...trace.toolCalls.map((toolCall) => toolCall.name),
          ]),
        ),
        reinforcementCount: Math.max(
          1,
          Number(existing?.reinforcementCount || 1),
        ),
  ...
  relevantLearnings(input: {
    ...
    return this.state.learnings
      ...
      .map((learning) => ({
        learning,
        score: relevanceScore(learning, haystack, contextFile),
      }))
      .filter((entry) => entry.score > 0)
      .sort(
        (left, right) =>
          right.score - left.score ||
          right.learning.updatedAt - left.learning.updatedAt,
      )
      .slice(0, limit)
  ...
  reviewSnapshot(chatId: string) {
    ...
    return deepClone({
      ...
      activeLearnings: this.state.learnings
        .filter((learning) => learning.status === 'active')
        .slice(0, 20),
      traceSummary: {
        userMessages: trace.messages.filter(
          (message) => message.role === 'user',
        ).length,
        assistantMessages: trace.messages.filter(
  ...
  private ensureTrace(chatId: string, metadata: TraceMetadata = {}) {
    ...
    if (!trace) {
      trace = {
        ...
        contextExtensionFile: metadata.contextExtensionFile
          ? path.basename(metadata.contextExtensionFile)
          : undefined,
    ...
    if (metadata.contextExtensionFile)
      trace.contextExtensionFile = path.basename(metadata.contextExtensionFile);
    if (metadata.extensionFiles?.length)
      trace.extensionFiles = Array.from(
        new Set([
          ...trace.extensionFiles,
          ...normalizedExtensionFiles(metadata.extensionFiles),
        ]),
      );
    trace.updatedAt = Date.now();
    return trace;
  ...
  private normalizedTrace(trace: Partial<StoredLearningTrace>) {
    return {
      ...
      contextExtensionFile: trace.contextExtensionFile
        ? path.basename(String(trace.contextExtensionFile))
        : undefined,
      extensionFiles: normalizedExtensionFiles(trace.extensionFiles),
      messages: Array.isArray(trace.messages)
        ? trace.messages.map((message) => ({
            role:
              message.role === 'user' || message.role === 'system'
                ? message.role
                : 'assistant',
            content: limitedText(message.content),
            createdAt: Number(message.createdAt || Date.now()),
          }))
        : [],
      toolCalls: Array.isArray(trace.toolCalls)
        ? trace.toolCalls.map((toolCall) => ({
            id: String(toolCall.id || `${Date.now()}`),
            name: limitedText(toolCall.name, 100),
            startedAt: Number(toolCall.startedAt || Date.now()),
            endedAt: toolCall.endedAt ? Number(toolCall.endedAt) : undefined,
            inputSummary: sanitizedData(toolCall.inputSummary),
            outputSummary: sanitizedData(toolCall.outputSummary),
            ok: toolCall.ok,
            error: toolCall.error
              ? limitedText(toolCall.error, 2_000)
              : undefined,
          }))
        : [],
      extensionEvents: Array.isArray(trace.extensionEvents)
        ? trace.extensionEvents.map((event) => ({
            kind:
              event.kind === 'validate_extension' ||
              event.kind === 'remove_extension' ||
              event.kind === 'install_extension'
                ? event.kind
                : 'write_extension',
            filename: event.filename
              ? path.basename(String(event.filename))
              : undefined,
            extensionId: event.extensionId
              ? String(event.extensionId)
              : undefined,
            commandIds: Array.isArray(event.commandIds)
              ? event.commandIds.map(String)
              : undefined,
            ok: Boolean(event.ok),
            error: event.error ? limitedText(event.error, 2_000) : undefined,
            createdAt: Number(event.createdAt || Date.now()),
            details: sanitizedData(event.details),
          }))
        : [],
      statusEvents: Array.isArray(trace.statusEvents)
  ...
  private normalizedLearning(learning: Partial<StoredLearning>) {
    ...
    return {
      ...
      appliesWhen: learning.appliesWhen
        ? limitedText(learning.appliesWhen, 300)
        : undefined,
      ...
      confidence:
        learning.confidence === 'low' || learning.confidence === 'high'
          ...
          : 'medium',
      status:
        learning.status === 'active' || learning.status === 'rejected'
          ? learning.status
          : 'observed',
      evidence: limitedText(learning.evidence, 500),
      evidenceChatIds: Array.isArray(learning.evidenceChatIds)
        ? learning.evidenceChatIds.map(String)
        : [],
      extensionFiles: normalizedExtensionFiles(learning.extensionFiles),
      toolNames: Array.isArray(learning.toolNames)
        ? learning.toolNames
            .map((toolName) => limitedText(toolName, 100))
            .filter(Boolean)
        : [],
  ...
  private trackOpenToolCall(traceId: string, name: string, toolCallId: string) {
    ...
    this.openToolCallIds.set(key, [
      ...(this.openToolCallIds.get(key) || []),
      toolCallId,
    ]);
  }
  ...
  private finishOpenToolCall(
    traceId: string,
    name: string,
    toolCallId: string,
  ) {
    const key = `${traceId}:${name}`;
    const remaining = (this.openToolCallIds.get(key) || []).filter(
      (id) => id !== toolCallId,
    );
  ...
  private async save() {
    ...
    await Promise.all([
      fs.writeFile(
        this.paths.tracesPath,
        JSON.stringify({ version: 1, traces: this.state.traces }, null, 2),
      ),
      fs.writeFile(
        this.paths.learningsPath,
        renderLearningsMarkdown(this.state.learnings),
      ),
    ]);
  }
}
...
function strongestConfidence(
  left: LearningConfidence,
  right?: LearningConfidence,
) {
...
function relevanceScore(
  ...
  if (
    contextExtensionFile &&
    learning.extensionFiles.includes(contextExtensionFile)
  )
    score += 4;
  ...
  if (
    ...
    haystack.includes(learning.appliesWhen.toLowerCase())
  )
    score += 2;
  return score;
}
...
function qualifiesUserLearning(
  input: ReviewLearningInput & { summary: string; kind: LearningKind },
) {
  const haystack =
...
function preservesLearning(
  ...
  if (containsBlockedLearningPattern(haystack)) return false;
  const relatedTrace = traces.find((trace) =>
    learning.evidenceChatIds.includes(trace.chatId),
  );
  return relatedTrace
    ? relatedTrace.toolCalls.length > 0 ||
        relatedTrace.extensionEvents.length > 0
    : learning.reinforcementCount >= 1;
}

function containsBlockedLearningPattern(haystack: string) {
  return /extensionsdir|application support\/nvm|reference existing|pattern matching|get inspired|local extension implementations|stored in \/|extension files live|media-compressor|\/users\//.test(
    haystack,
  );
}

function parseLearningsContent(
  text: string,
): { learnings?: Partial<StoredLearning>[] } | null {
  if (!text) return null;
  ...
  for (const block of blocks) {
    ...
    for (const rawLine of lines) {
      ...
      if (line.startsWith('When: '))
        ...
        current.confidence = normalizeConfidence(
...
function renderLearningsMarkdown(learnings: StoredLearning[]) {
  ...
  for (const section of sections) {
    ...
    const items = learnings.filter(
      (learning) =>
        learning.status === 'active' && learning.kind === section.kind,
    );
    if (!items.length) {
      lines.push('_None_', '');
      continue;
    ...
    for (const learning of items) {
      ...
      if (learning.keywords.length)
        lines.push(`  Keywords: ${learning.keywords.join(', ')}`);
...
async function migrateLegacyLearningsFile(paths: LocalLearningStorePaths) {
  ...
  const current = await fs
    .readFile(paths.learningsPath, 'utf8')
    .catch(() => '');
  if (current) {
  ...
  const legacy = await fs
    .readFile(paths.legacyLearningsPath, 'utf8')
    .catch(() => '');
  if (!legacy) return;
  try {
    const parsed = JSON.parse(legacy) as Partial<LearningStoreState>;
    const learnings = Array.isArray(parsed.learnings)
      ? parsed.learnings.map((learning) =>
          normalizedStoredLearningForMigration(learning),
        )
      : [];
...
function normalizedStoredLearningForMigration(
  learning: Partial<StoredLearning>,
): StoredLearning {
  const kind =
    learning.kind === 'workflow' || learning.kind === 'preference'
      ? learning.kind
  ...
  return {
    id: String(
      learning.id || `${Date.now()}-${Math.random().toString(36).slice(2, 10)}`,
    ),
    fingerprint: learningFingerprint(kind, summary),
    kind,
    summary,
