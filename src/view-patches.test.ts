...
test('patchCommandView supports prepend and append modes', () => {
  assert.deepEqual(patchCommandView(baseView, { mode: 'prepend', items: [{ id: 'zero', title: 'Zero' }] }).items, [
    { id: 'zero', title: 'Zero' },
    { id: 'one', title: 'One' },
    { id: 'two', title: 'Two' },
  ])

  assert.deepEqual(patchCommandView(baseView, { mode: 'append', items: [{ id: 'three', title: 'Three' }] }).items, [
    { id: 'one', title: 'One' },
    { id: 'two', title: 'Two' },
...
test('patchCommandView applies item patches to views that omitted items', () => {
  ...
  assert.deepEqual(patchCommandView(loadingView, { mode: 'replace', items: [{ id: 'one' }] }).items, [
    { id: 'one', title: 'one' },
  ])
