...
- Add `appearance?: ExtensionItemAppearance` to `ExtensionActionContribution` and `ExtensionCommand` in `src/app/resources/nevermind-extension-api.d.ts`.
- Propagate command `appearance` during command registration in `src/app/electron/main.ts`.
- Keep renderer styling centralized in `src/app/palette/styles.css` using the existing `data-foreground` attribute and `--item-foreground-*` tokens.
