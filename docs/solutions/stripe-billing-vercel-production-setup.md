...
- `src/backend/src/pages/api/billing/checkout.ts`
- `src/backend/src/pages/api/billing/portal.ts`
- `src/backend/src/pages/api/billing/webhook.ts`
- `src/backend/src/lib/billing.ts`
- `src/backend/src/lib/cost.ts`
...
- Git worktrees do not inherit ignored local files such as `src/backend/.env` or `.vercel/` from the canonical checkout.
...
When doing src/backend/Vercel work from a git worktree, remember that ignored local configuration does not come along with the worktree:
...
Keywords: Stripe, billing, Checkout, Billing Portal, webhook, webhooks, event destination, `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_SUBSCRIPTION_TIERS`, `STRIPE_TOP_UP_PACKS`, `CREDIT_USD`, `CREDIT_MARKUP`, `MONTHLY_FREE_CREDITS`, `price_`, product ID vs price ID, `checkout.session.completed`, `invoice.paid`, `payment_intent.succeeded`, `customer.subscription.updated`, `customer.subscription.deleted`, Vercel env, Vercel deploy, outdated Vercel CLI, worktree env, `.vercel`, `src/backend/.env`, `api.nvm.fyi`, `nvm.fyi` redirect.
