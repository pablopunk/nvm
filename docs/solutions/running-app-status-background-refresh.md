...
Root app results render a running/open indicator via `runningAppClassName` in `src/app/palette/App.tsx`. That status is visual decoration; it must never gate user-visible search, input, command execution, or palette reveal.
...
- `src/app/palette/preload-api.ts`
