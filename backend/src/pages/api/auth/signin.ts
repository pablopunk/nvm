...
type SignInDependencies = {
  ...
  createState: typeof createProductionState;
  authorizationUrl: (input: { provider: 'authkit'; clientId: string; redirectUri: string; state: string }) => string;
};

export function createSignInHandler(overrides: Partial<SignInDependencies> = {}): APIRoute {
  const dependencies: SignInDependencies = {
    rateLimit: rateLimitIp,
    createState: createProductionState,
    authorizationUrl: (input) => workos.userManagement.getAuthorizationUrl(input),
    ...overrides,
  };

  return async ({ url, request, redirect }) => {
    ...
    const state = await dependencies.createState(url.searchParams.get('return_to'));
    if (!state) return new Response('Authentication is temporarily unavailable', { status: 503 });
    const authorizationUrl = dependencies.authorizationUrl({
      provider: 'authkit',
